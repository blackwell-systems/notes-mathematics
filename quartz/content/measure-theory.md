...
**Worked example.** Take the three-level staircase on $[0,4]$ (with Lebesgue measure),

$$
s = 2 \cdot \mathbf{1}_{[0,1)} + 5 \cdot \mathbf{1}_{[1,3)} + 1 \cdot \mathbf{1}_{[3,4)}
$$

The three sets are $A_1 = [0,1)$ with $\mu(A_1) = 1$, $A_2 = [1,3)$ with $\mu(A_2) = 2$, and $A_3 = [3,4)$ with $\mu(A_3) = 1$. The integral is just height times width, summed:

$$
\int s \, d\mu = 2\cdot 1 + 5 \cdot 2 + 1 \cdot 1 = 13
$$

Notice this is exactly the area under the staircase, so for a step function the Lebesgue and Riemann answers already agree; the power of the Lebesgue definition is that the $A_i$ are allowed to be *any* measurable sets, not just intervals. That is what lets the same formula handle the Dirichlet function below, where the "steps" sit on $\mathbb{Q}$ and its complement.
...
**Worked example (norms and Cauchy-Schwarz on $[0,1]$).** Take $f(x) = x$ on $[0,1]$ with Lebesgue measure. Its three standard norms are

$$
\|f\|_1 = \int_0^1 x \, dx = \tfrac{1}{2}, \qquad
\|f\|_2 = \left( \int_0^1 x^2 \, dx \right)^{1/2} = \sqrt{\tfrac{1}{3}} \approx 0.577, \qquad
\|f\|_\infty = \sup_{[0,1]} x = 1
$$

so here $\|f\|_1 \le \|f\|_2 \le \|f\|_\infty$. This ordering is not an accident: on a space of total measure $1$ (a probability space), the $L^p$ norms increase with $p$, a direct consequence of Holder's inequality.

Now check Cauchy-Schwarz ($p = q = 2$) against a second function $g(x) = x^2$. The inner product is
...
$$
\|f\|_2 \, \|g\|_2 = \sqrt{\tfrac{1}{3}} \cdot \sqrt{\tfrac{1}{5}} = \sqrt{\tfrac{1}{15}} \approx 0.258
$$

Indeed $0.25 \le 0.258$, so Cauchy-Schwarz holds, and the two sides are close because $x$ and $x^2$ are nearly "parallel" (both increasing, no sign changes) on $[0,1]$. Equality would require $g$ to be a scalar multiple of $f$, which $x^2$ is not, so the inequality is strict.
...
**Worked example (both orders agree).** Integrate $f(x, y) = x + y^2$ over the unit square $[0,1] \times [0,1]$. This does not factor into a product of a function of $x$ and a function of $y$, so the order-independence is a genuine check, not automatic. Integrating over $y$ first:

$$
\int_0^1 \left( \int_0^1 (x + y^2) \, dy \right) dx = \int_0^1 \left( x + \tfrac{1}{3} \right) dx = \tfrac{1}{2} + \tfrac{1}{3} = \tfrac{5}{6}
$$

Integrating over $x$ first:

$$
\int_0^1 \left( \int_0^1 (x + y^2) \, dx \right) dy = \int_0^1 \left( \tfrac{1}{2} + y^2 \right) dy = \tfrac{1}{2} + \tfrac{1}{3} = \tfrac{5}{6}
$$

Both orders give $\tfrac{5}{6}$, as Tonelli guarantees (the integrand is non-negative on the square, so no integrability check is even needed here).
...
**Worked example (conditional expectation is block-wise averaging).** Roll a fair die: $\Omega = \{1, 2, 3, 4, 5, 6\}$, each outcome with probability $\tfrac{1}{6}$, and let $X$ be the face value. Let $\mathcal{G}$ be the sub-sigma-algebra generated by the partition into odd and even outcomes, $\{1,3,5\}$ and $\{2,4,6\}$; knowing $\mathcal{G}$ tells you only the parity, not the exact face. Then $E[X \mid \mathcal{G}]$ is constant on each block, equal to the average of $X$ over that block:

$$
E[X \mid \mathcal{G}](\omega) = \begin{cases} \dfrac{1+3+5}{3} = 3 & \omega \text{ odd} \\[2mm] \dfrac{2+4+6}{3} = 4 & \omega \text{ even} \end{cases}
$$

This is a random variable (a function of the outcome), not a single number: it reports $3$ if the roll is odd and $4$ if even. Check the defining property on the block $G = \{1,3,5\}$:

$$
\int_G E[X \mid \mathcal{G}] \, dP = 3 \cdot P(G) = 3 \cdot \tfrac{1}{2} = \tfrac{3}{2}, \qquad \int_G X \, dP = (1 + 3 + 5)\cdot \tfrac{1}{6} = \tfrac{9}{6} = \tfrac{3}{2}
$$

The two agree, as required. And averaging $E[X \mid \mathcal{G}]$ itself recovers the ordinary expectation (the **tower property** $E[E[X \mid \mathcal{G}]] = E[X]$):

$$
E\big[E[X \mid \mathcal{G}]\big] = 3 \cdot \tfrac{1}{2} + 4 \cdot \tfrac{1}{2} = \tfrac{7}{2} = 3.5 = \frac{1+2+3+4+5+6}{6} = E[X]
$$
...
Almost sure and $L^p$ convergence are incomparable: neither implies the other in general. ($L^p$ does not imply a.s.: the "typewriter" sequence, where an indicator of width $1/2^k$ sweeps across $[0,1]$ and then restarts from the left again and again, has $\|f_n\|_p \to 0$ yet, because every point of $[0,1]$ is covered infinitely often and also missed infinitely often, $f_n(x)$ oscillates between $1$ and $0$ and converges at no single point. Conversely, a.s. convergence does not imply $L^p$, because the mass may escape to infinity, as with $f_n = n \cdot \mathbf{1}_{[0,1/n]}$, which tends to $0$ pointwise but has $\|f_n\|_1 = 1$ for every $n$.)
