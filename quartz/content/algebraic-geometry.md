...
![The algebra-geometry dictionary: a two-column table pairing geometric objects on the left with algebraic objects on the right. The V-of operation sends an ideal to its common zeros; the I-of operation sends a variety to the polynomials vanishing on it. Paired rows read: a variety V and its ideal I of V; a smaller variety and a bigger ideal, so inclusion is order-reversing; an irreducible variety and a prime ideal; a union and a product or intersection of ideals; an intersection and a sum of ideals; the empty variety and the whole ring containing 1; the dimension of V and the Krull dimension of the coordinate ring. Hilbert's Nullstellensatz makes the pairing a genuine bijection over an algebraically closed field.](./media/ag-algebra-geometry-dictionary.png)

Reading the table, notice the pattern in the second row: the correspondence is **order-reversing**. A bigger shape satisfies fewer polynomial constraints, so a larger variety has a smaller ideal. This inclusion-flipping is the fingerprint of the dictionary, and it is why questions phrased one way (is this variety empty?) turn into their mirror image on the other side (does this ideal contain everything, including $1$?).
...
![The blowup of the node V of y squared minus x squared. Left panel: in the x-y plane the two branches y equals x and y equals minus x cross at the origin, which is marked as a singular point where both branches meet and the tangents are tangled. Right panel: after the change of coordinates x equals u and y equals u v, the coordinate v equals y over x records the direction of approach. The branch y equals x lifts to the horizontal line v equals plus one and the branch y equals minus x lifts to v equals minus one, two separate horizontal lines. The vertical dashed line u equals zero is the exceptional divisor, the blown-up origin, a whole line of directions. The two branches now meet this divisor at two different points, the point v equals plus one and the point v equals minus one, so the crossing has been separated and resolved.](./media/ag-blowup-node.png)

The figure shows exactly this. On the left, the two branches are welded together at the origin, where the tangent direction is ambiguous. On the right, the extra coordinate $v = y/x$ records *which direction* you came in on, and that is precisely the information the crossing destroyed. The branch $y=x$ has $v = +1$ and the branch $y = -x$ has $v = -1$ everywhere, so once we track $v$ the two branches never touch: they cross the exceptional divisor $u = 0$ at two different heights. The single tangled point has been pulled apart into two clean smooth points.
